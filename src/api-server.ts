...
function getRequestAuth(req: http.IncomingMessage): string {
  const auth = req.headers.authorization || ''
  if (Array.isArray(auth)) return auth[0] || ''
  return auth
}

function getRequestApiKey(req: http.IncomingMessage): string {
...
function createModelsPayload(): unknown {
...
export function responsesPayloadToChatCompletion(payload: any, fallbackModel?: string): Record<string, unknown> {
...
async function writeChatCompletionResponse(
...
function writeSseData(res: http.ServerResponse, payload: unknown): void {
